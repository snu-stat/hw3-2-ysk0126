...
따라서 R과 tidyverse가 포함된 환경에서 시작하며, 이후 시스템 라이브러리, Miniconda, Python 패키지, R 패키지를 추가로 설치한다. 
`RETICULATE_PYTHON` 환경변수는 Quarto 문서 안에서 R의 `reticulate` 패키지가 사용할 Python 실행 파일을 `/opt/conda/envs/r-reticulate/bin/python`으로 고정한다. 
이를 통해 로컬 환경과 Binder 환경에서 R-Python 연동 결과가 달라지는 것을 줄일 수 있다.

정리하면, 이 Dockerfile은 숙제 3의 Quarto 문서와 자동 생성된 Jupyter Notebook 파일을 Binder에서 재현 가능하게 실행하기 위한 컨테이너 환경 설정 파일이다.
...
여기서 `contents: write`는 기존처럼 `gh-pages` branch에 결과물을 배포하기 위해 필요하다. 
새로 추가한 `packages: write`는 GitHub Actions가 GitHub Container Registry에 Docker image를 push할 수 있도록 허용하는 권한이다. 
`docker/login-action@v3` 단계에서는 `ghcr.io`에 로그인하며, 비밀번호로 GitHub Actions에서 자동으로 제공하는 `${{ secrets.GITHUB_TOKEN }}`을 사용한다. 
그다음 `docker/build-push-action@v5` 단계에서 저장소 최상위 디렉토리의 `Dockerfile`을 이용해 Docker image를 빌드하고, 이를 `ghcr.io/${{ github.repository_owner }}/my-r-env:latest`라는 태그로 GHCR에 push한다.

따라서 이 workflow가 성공하면 GitHub Pages 배포가 수행된 뒤, 같은 저장소의 `Dockerfile`을 기반으로 Binder에서 사용할 수 있는 Docker container image도 자동으로 생성된다.
처음에는 문제 예시처럼 `ghcr.io/${{ github.repository_owner }}/my-r-env:latest`를 사용하였다. 
그러나 GitHub Classroom 저장소의 owner가 개인 계정이 아니라 수업 organization인 `snu-stat`였기 때문에, workflow가 `ghcr.io/snu-stat/my-r-env:latest`로 push를 시도하였다. 
이 과정에서 organization package에 대한 `write_package` 권한이 없어 `permission_denied: write_package` 오류가 발생하였다. 
따라서 실제 workflow에서는 image가 현재 과제 저장소와 연결되도록 `ghcr.io/${{ github.repository }}:latest`를 사용하였다. 
이 경우 image 주소는 `ghcr.io/snu-stat/hw3-2-ysk0126:latest`가 되며, Binder 링크도 이에 맞추어 작성하였다.
...
숙제 2와 마찬가지로 GitHub Pages를 사용하여 저장소를 웹페이지처럼 활용하되, 이번에는 단순히 `.ipynb` 파일을 내려받는 링크만 제공하는 것이 아니라 Binder를 통해 온라인에서 주피터 노트북을 바로 실행할 수 있는 링크를 추가하였다.

이전 단계에서 작성한 GitHub Actions workflow는 `README.md`를 `index.md`로 복사한 뒤 GitHub Pages에 배포한다. 
따라서 저장소 최상위 디렉토리에 있는 `README.md`는 GitHub Pages의 첫 화면 역할을 한다.

저장소 최상위 디렉토리에 작성한 `README.md`의 내용은 다음과 같다.
...
여기서 `ysk0126`은 제출자의 GitHub 사용자 아이디이다. 
따라서 다른 사용자가 같은 구조를 사용할 경우에는 Binder 링크의 `ysk0126` 부분을 자신의 GitHub 사용자 아이디로 바꾸어야 한다. 
또한 GitHub Actions workflow의 Docker image push 단계에서 사용한 image tag가 `ghcr.io/${{ github.repository_owner }}/my-r-env:latest`였으므로, Binder 링크의 GHCR owner 부분도 이 image owner와 일치해야 한다.

GitHub Container Registry의 image 주소는 일반적으로 `ghcr.io/NAMESPACE/IMAGE_NAME:latest`와 같은 형태를 사용하며, `NAMESPACE`는 개인 계정 또는 조직 이름에 해당한다. 
이 과제에서는 workflow에서 만든 image가 `ghcr.io/<유저명>/my-r-env:latest`이므로 Binder 링크에서는 다음과 같은 형태를 사용하였다.

```text
https://mybinder.org/v2/ghcr/<유저명>/my-r-env/latest?filepath=hw03.ipynb
```

이 링크를 클릭하면 Binder가 GHCR에 push된 Docker image를 사용하여 온라인 실행 환경을 만들고, 그 안에서 `hw03.ipynb` 노트북을 열 수 있다. 
따라서 사용자는 별도로 R, Python, Quarto, 패키지를 설치하지 않아도 웹브라우저에서 분석 노트북을 실행할 수 있다.

작업을 마친 뒤에는 `README.md`, `hw03.qmd`, `hw03.html`, `hw03.ipynb`, `Dockerfile`, `.github/workflows/publish.yml`, `student.yml`이 모두 숙제 3 GitHub 저장소에 올라가 있는지 확인하였다. 
그다음 GitHub Actions 탭에서 workflow가 성공적으로 실행되었는지 확인하고, GitHub Pages 주소로 접속하여 HTML 리포트 링크와 Binder 링크가 정상적으로 보이는지 확인하였다.
